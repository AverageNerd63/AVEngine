    ...
    if (!fence->is_signaled) {
        ...
        switch (result) {
            case VK_SUCCESS:
                ...
                return TRUE;
            case VK_TIMEOUT:
                AVWARN("vk_fence_wait - Timed out");
                break;
            case VK_ERROR_DEVICE_LOST:
                AVERROR("vk_fence_wait - VK_ERROR_DEVICE_LOST.");
                break;
            case VK_ERROR_OUT_OF_HOST_MEMORY:
                AVERROR("vk_fence_wait - VK_ERROR_OUT_OF_HOST_MEMORY.");
                break;
            case VK_ERROR_OUT_OF_DEVICE_MEMORY:
                AVERROR("vk_fence_wait - VK_ERROR_OUT_OF_DEVICE_MEMORY.");
                break;
            default:
                AVERROR("vk_fence_wait - An unknown error has occurred.");
                break;
        }
    } else {
